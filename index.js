...
function S3Storage (opts) {
  ...
  switch (typeof opts.key) {
    case 'function':
      ...
      break
    case 'string':
      this.getKey = staticValue(opts.key)
      break
    case 'undefined':
  ...
  switch (typeof opts.contentType) {
    case 'function':
      ...
      break
    case 'string':
      this.getContentType = staticValue(opts.contentType)
      break
    case 'undefined':
      this.getContentType = defaultContentType
      break
    default:
      throw new TypeError('Expected opts.contentType to be undefined, string or function')
  }
  ...
  this.transformers.map(function (transformer) {
    switch (typeof transformer.id) {
      case 'string':
        break
      default:
        throw new TypeError('Expected opts.transformer[].id to be string')
    }
    ...
    switch (typeof transformer.key) {
      case 'function':
        break
      case 'string':
        transformer.key = staticValue(transformer.key)
        break
      case 'undefined':
        transformer.key = defaultKey()
        break
      default:
        throw new TypeError('Expected opts.transformer[].key to be unedefined, string or function')
    }
    ...
    switch (typeof transformer.transform) {
      case 'function':
        break
      default:
        throw new TypeError('Expected opts.transformer[].transform to be function')
    }

    switch (typeof transformer.contentType) {
      case 'function':
        transformer.getContentType = transformer.contentType
        break
      case 'string':
        transformer.getContentType = staticValue(transformer.contentType)
        break
      case 'undefined':
        transformer.getContentType = staticValue(undefined)
        break
      default:
        throw new TypeError('Expected opts.transformer[].contentType to be undefined, string or function')
    }
...
module.exports = function multerS3 (opts) {
